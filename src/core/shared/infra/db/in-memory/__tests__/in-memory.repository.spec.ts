...
describe('InMemoryRepository Unit Tests', () => {
  ...
  test('should returns all entities', async () => {
    const entity = new StubEntity({ name: 'name test', price: 5 });
    await repo.insert(entity);

    const entities = await repo.findAll();

    expect(entities).toStrictEqual(entities);
  });

  test('should throws erros on update when entity not found', () => {
    const entity = new StubEntity({ name: 'test name', price: 5 });
    expect(repo.update(entity)).rejects.toThrow(
      new NotFoundError(entity.entity_id, StubEntity),
    );
  });

  test('should throws erros on update when entity not found', async () => {
    const entity = new StubEntity({ name: 'test name', price: 5 });
  ...
  test('should throws erros on delete when entity not found', async () => {
    const uuid = new Uuid();
